...
- **API responses**: **NetworkOnly**. Dexie is the offline store; the API exists only to reconcile against the *current* server state, so a stale hit is worse than a failed request. `defaultCache` would otherwise cache `/api/*` NetworkFirst, which let a flaky connection feed `/api/recipes/sync` a stale server snapshot for the server-wins reconcile to act on.
