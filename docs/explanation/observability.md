...
- **Identity:** `identifyUser` on login attaches `email` / `name` / `image` (when the provider supplies them) and `locale`; `resetIdentity` on logout. `useTelemetryIdentity` is mounted twice — in `ClientShell` for the app, and in `app/external-auth/layout.tsx` for the auth host. The auth pages are served from a **different origin** (`auth.recipai.pp.ua`) and PostHog persistence is per-origin, so they start with their own anonymous distinct id; without an `identify` there it never merged, and every trip through account-linking or device sign-in became a separate person. Since account-linking is exactly where someone connects an extra account, that is what made one user with several connected accounts look like several users. The cross-origin part needs no solving: PostHog merges on the server by distinct id, so identifying the auth host's anonymous id as the same user id folds the two together retroactively. Note `locale` is only set when the first path segment actually is one — under `/external-auth` it is not, and writing it anyway would overwrite a real locale with `"external-auth"`.
