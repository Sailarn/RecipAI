...
sequenceDiagram
    ...
    Client->>Poll: polling, 3s backing off to 30s
...
**Poll cadence and the give-up deadline.** `use-parse-job-watcher` starts at 3 s and multiplies the gap by 1.5 after each `processing` response, capped at 30 s — most parses land in the first few polls, and a long-running job gains nothing from being asked every 3 s (each poll is a server round-trip, and through the proxy a maintenance-config read). A job the server still reports as `processing` **15 minutes** after the watcher picked it up is abandoned: the watcher claims the completion, drops the saved job id, writes a `failed` parse-history entry with reason "Parsing timed out — the job never finished," and surfaces an error toast, so a stuck parse leaves a record instead of polling for the lifetime of the tab. Network errors are exempt — offline is a normal state here, so they keep retrying every 5 s and never count towards the deadline.
