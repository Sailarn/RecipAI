...
| `hooks/use-category-label.ts` | Display label for a recipe category. The stored value stays English — it's a fixed enum in Dexie/Postgres that the AI parse prompt names — so only the label is localised, and unknown values pass through unchanged. |
| `app/external-auth/intl-provider.tsx` | next-intl context for the `external-auth` tree, which sits outside `app/[locale]` and so never gets a locale from middleware. Each page passes the `?locale=` the app put in the link. |
