...
| `GET` | `/api/recipes/[id]/public` | None (public by design) | Client-fetchable counterpart to `getPublicRecipe()` — same isPublic-only scope, `404` otherwise. This is now **the** way a shared recipe is resolved: `RecipeDetail` calls it whenever a recipe isn't on the device and isn't the signed-in user's own. The recipe page no longer server-fetches a `publicRecipe` prop — blocking the shell on Supabase made every deep link pay for the rare share case, when the common one (your own recipe, already in Dexie) needs no server data at all. `getPublicRecipe()` is still used in `generateMetadata` for OpenGraph tags. |
...
| `GET` | `/api/build` | None (public by design) | Returns `{ buildId }` — the build the **server** is serving right now, `force-dynamic` + `Cache-Control: no-store`. `reportBuildFreshness()` (`lib/pwa/build-freshness.ts`) compares it against the id compiled into the running bundle to detect a document left over from a previous deploy, emitting `stale_document_detected` and pulling a fresh service worker. It must be asked over HTTP rather than read locally: any value baked into the bundle would be as stale as the bundle itself. The service worker routes `/api/*` through `NetworkOnly`, so the answer is never cached. Unauthenticated on purpose — staleness is a property of the document, not the user, and has to be answerable before anything else resolves. Maintenance mode does gate it like any other `/api/*` route; the caller treats a non-OK response as "no answer" rather than as staleness. |
