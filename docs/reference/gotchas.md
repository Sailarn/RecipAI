...
**`initData` can populate *after* the SDK script's `load` event, not just before.** Confirmed via PostHog: `telegram_mini_app_launched` (fired inside `initializeWebApp()`, gated on `getTelegramWebApp()` returning non-empty) consistently never fired for deep-link launches (a `recipe_<id>` startapp param) while firing reliably for plain reopens — on iOS, in the traces seen. Telegram's native layer can inject `initData` onto the WebApp object *after* the script's own `load` event, so a single check at that moment sees an "empty" object and — with no retry — `TelegramProvider.webApp` stays `undefined` for the rest of the session, even though `isTelegramEnvironment()` (hash-based) already knows it's Telegram. Silent downstream: `useTelegramAutoSignIn` never attempts sign-in with an `undefined` webApp (no account auto-created for a new user), and `ProfileAuth`'s `awaitingTelegramAutoSignIn` spins forever since `authStatus` never leaves `"idle"`. No Sentry/Axiom signal either, since nothing throws — `TelegramProvider` now polls `getTelegramWebApp()` every 200ms for ~5s after the initial miss instead of giving up on the first check.
