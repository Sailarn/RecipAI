...
`POST /api/parse-queue` serves a prior parse for the same URL by matching `parse_jobs.normalized_url` (`normalizeSourceUrl`) + the current `PARSER_VERSION`. Three non-obvious rules: (1) **URL normalization must canonicalize Instagram** — `/reel/X`, `/reels/X`, `/p/X`, `/tv/X` are the same media, so they collapse to `instagram.com/reel/X`; without this, `/reels/` misses the `/reel/` cache and re-parses. (2) **Empty parses (0 ingredients and 0 steps) are failed, not cached**, and the lookup also excludes empty results — a failed extraction must never poison a URL's cache. (3) **The image is uploaded to ImageKit at parse time**, not at save time, because the stored `result.imageUrl` is what a *cached* hit serves and source CDN URLs (Instagram's) expire within hours. Bump `PARSER_VERSION` to invalidate the whole cache after a prompt/model change; to heal individual stale-image rows, clear their `parser_version` so they re-parse fresh on next import.
