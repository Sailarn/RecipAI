...
**Vocab delta-sync deliberately overlaps its watermark.**
`pullVocab` (`lib/db/sync-vocab.ts`) stores the max `updatedAt` it has seen in `localStorage["ingredientsSyncedAt"]` and re-pulls with `?since=<watermark>` → `gt(ingredients.updatedAt, since)`. Before migration `0016`, `ingredients.updated_at` was timezone-naive; timestamp conversion plus strict `>` could skip the boundary row and leave one ingredient stale locally. The columns are now `timestamptz`, but two client guards remain as defense-in-depth: the pull overlaps the watermark by `WATERMARK_OVERLAP_MS` (60s), and a change in `NEXT_PUBLIC_APP_VERSION` forces a full re-pull (tracked in `localStorage["ingredientsSyncedVersion"]`). `bulkPut` is an idempotent upsert, so re-fetching the overlap is harmless.

**Data API grants and RLS are separate security layers.**
RLS is off on all 13 Postgres tables and there are no policies. A matching table `GRANT` to `anon` or `authenticated` would therefore expose every row allowed by that grant through the Data API; with RLS enabled, table grants permit operations while policies filter which rows are accessible. The app itself never uses a public Supabase key: it connects through `DATABASE_URL` as the table-owning `postgres` role (`db/index.ts`), which bypasses RLS while `relforcerowsecurity` is off. The live project has previously been operated with zero `anon`/`authenticated` grants, but the repository migrations contain no `REVOKE` or `ALTER DEFAULT PRIVILEGES`, so that state must be verified per deployment and is not reproduced from source. Before granting client access, add repository-managed least-privilege grants, enable RLS, and write per-user policies; enabling RLS as deny-all first makes future grant mistakes fail closed without blocking the current owner connection.

**The parse result cache keys on a *normalized* URL and attempts image durability before caching.**
`POST /api/parse-queue` serves a prior parse for the same URL by matching `parse_jobs.normalized_url` (`normalizeSourceUrl`) + the current `PARSER_VERSION`. Three non-obvious rules: (1) **URL normalization must canonicalize Instagram** — `/reel/X`, `/reels/X`, `/p/X`, `/tv/X` are the same media, so they collapse to `instagram.com/reel/X`; without this, `/reels/` misses the `/reel/` cache and re-parses. (2) **Incomplete parses are failed, not cached** — both ingredients and instructions must be non-empty, and the lookup enforces the same condition so a failed extraction cannot poison the cache. (3) **The process route attempts an ImageKit upload at parse time**, not save time, because the stored `result.imageUrl` is what a cached hit serves and source CDN URLs (Instagram's especially) expire within hours. Upload failure is non-fatal and leaves the original source URL in the cached result, so a broken cached image can still indicate a failed durability upload — the failure is never swallowed, but an unreachable source CDN is **logged rather than captured to Sentry** (see the Observability section for why). The server-side fetch of the source image (`lib/upload/imagekit.ts` and the client-save path in `lib/upload/upload-image-source.ts`) must send **browser-like headers** via `imageFetchHeaders` — Instagram's `cdninstagram.com`/`fbcdn.net` CDN 403s (or returns an HTML block page for) a bare Node `fetch`, which is what left recent Instagram recipes on an expiring URL with no ImageKit copy. Bump `PARSER_VERSION` to invalidate the whole cache after a prompt/model change; to heal individual stale-image rows, clear their `parser_version` so they re-parse fresh on next import.

**pgvector `<=>` is cosine distance, not similarity.**
The nearest-vocabulary query converts it with `similarity = 1 - distance`. Do not compare the raw `<=>` result to the similarity threshold. The server fetches the top two confirmed neighbors and accepts the best only when its similarity is at least `0.88` and it leads the runner-up by at least `0.02`. These values were calibrated for `multilingual-e5-small` with an untracked local probe sweep: same-language correct matches cluster ≥0.89 while generic queries top out at ~0.875, so `0.88` separates them. The earlier `0.82`/`0.08` pair was carried over from a different on-device model — its `0.08` gap rejected most correct matches that had a close sibling (e.g. `tomato` vs `tomato-cherry`). At the current vocabulary size this is an exact scan; add an HNSW/IVFFlat index only after measuring a larger dataset.
...
**Never hardcode feature/client route strings.**
At UI and app-client call sites, import shared URLs from `lib/routes.ts`. Bare strings like `"/recipes"` miss the locale prefix — `routes.recipes.list(locale)` yields `/<locale>/recipes`. Framework-owned routes and internal server-only endpoints do not need entries in this client route map.
...
Fixed: the upload route now falls through to a per-IP rate limit instead of a hard 401 when there's no session/token (`requireUploadAuthOrRateLimit`, `lib/upload/upload-auth.ts`) so anonymous edits work again; the resolve functions now report the failure (see the next entry for which ones reach Sentry); and a failed photo upload keeps the user on the form with a persistent error instead of navigating away. When a recipe-image "silently doesn't update," check for `phantomjs_fallback`-style masking first — this exact pattern (swallow the error, report success anyway) has bitten this app twice now.

**An unreachable source-CDN image is expected, and deliberately not reported.**
A parsed recipe's image lives on the origin site's CDN, which signs URLs, locks them to a host, or expires them within hours — so mirroring it to ImageKit legitimately comes back `403`/`404`/`5xx`, or fails at the network layer as undici's bare `TypeError: fetch failed`. Reporting those buried real upload breakage: the TikTok 403 alone was the top Sentry issue by volume, and the same incident fired again client-side from `/:locale/recipes/new`. `isSourceImageUnavailable()` (`lib/upload/source-image-failure.ts`) classifies them, and both call sites — `POST /api/parse-queue/process` and `resolve-recipe-images.ts` — log instead of capturing. Genuine defects (unsupported type, oversized file, ImageKit rejections) still go to Sentry, and the user is still told either way via `uploadFailed`. The check is **message-based on purpose**: the failure crosses an HTTP boundary (server throws → 400 body → browser rebuilds a fresh `Error`), so no error class survives the trip — keep `SOURCE_FETCH_FAILED` / `sourceFetchFailedMessage()` as the single source of those strings rather than retyping them.

**ImageKit's SDK rejects with a plain object, not an `Error`.**
`imagekit.deleteFile()` rejects with `{ message, help }`. An `error instanceof Error` check therefore reads an empty message, so `DELETE /api/images/delete`'s "already deleted is a no-op" and "transient internal error" branches never matched the real SDK — every already-deleted file became a Sentry-reported 500 titled `f` (the minified frame name). Use `imagekitErrorMessage()` in that route, and note that tests constructing `new Error("File does not exist")` will pass against the broken code — mock the object shape to actually cover it.

**Telegram photo sends degrade to text rather than being reported.**
`sendPhoto` can 400 when Telegram cannot fetch the photo URL, even a valid ImageKit one. The recipe is already saved by that point, so `sendRecipeCard()` in `POST /api/parse-queue/process` falls back to `sendTelegramMessage` with the same caption and button — the notification is delivered instead of dropped, and only a failing *text* send reaches Sentry.
...
**Units store canonical English codes and are localized at display, like modifiers.** `unit` holds `g`/`kg`/`ml`/`l`/`tsp`/`tbsp`/`pcs` regardless of the recipe's language — that's what `METRIC_UNITS_RULE` pins and what the servings calculator scales against. Rendering it raw was a real bug: a Ukrainian recipe read "30 g м'ята", "1 tsp розпушувач", and a source that literally said `1 шт.` stored `pcs`. `unitLabel(unit, locale)` (`lib/units.ts`) resolves the label; it keys off the **app locale**, not the recipe's language, because `Recipe` carries no language field. Because `unit` is free text (the edit form is a plain `<Input>`, and pre-enum rows exist), `unitLabel` matches through an alias table of English/Ukrainian/Russian spellings — keys are stored *collapsed* (lowercased, whitespace and periods stripped), so one entry covers `ч. л.` / `ч.л.` / `чл`. A value it cannot classify (`щіпка`, `bunch`) is returned untouched — never drop or mangle it. Nothing is written back, so this is safe to change without a migration.
...
**IndexedDB failures are announced, not silent.** Every read goes through Dexie, so a database that never opens (Safari private mode, storage blocked by policy) used to leave skeletons on screen forever with nothing in Sentry. `useDatabaseLifecycle` (mounted in `ClientShell`) opens the database explicitly and reports a failure with `source: "dexie-open"` plus a persistent toast. It also handles the two multi-tab cases: `versionchange` (another tab opened a newer schema — Dexie closes this connection, so the page must reload to work at all) and `blocked` (this tab is holding an old version open while another waits).

**A render error no longer takes the whole app down.** Every pushed page lives in the same client tree, so before `PageErrorBoundary` (wrapped around each `PageStack` entry) one bad render replaced the entire app with Next's generic error page and lost the navigation stack with it. Now the failing page shows a retry fallback while the shell, the pages underneath, and the toaster stay alive; retry remounts the page under a fresh key rather than replaying the state that threw. `app/[locale]/error.tsx` is the route-level backstop for errors thrown outside the stack, and `app/global-error.tsx` remains the last resort. The boundary reports through `captureError` — an error boundary is the one place that must swallow, so it also has to be the place that reports, which is why it is exempt from the re-throw rule in CLAUDE.md.
...
**Clipboard auto-detect is opt-in by browser, not by us — don't "fix" the missing suggestion.** The import field (`parse-form/url-field.tsx`) offers a Paste button *and* an automatic suggestion of a copied link, but the automatic half only ever fires in Chromium, and only after the user has granted `clipboard-read`. `useClipboardLink` checks `navigator.permissions.query({ name: "clipboard-read" })` and bails unless the state is already `granted`, on purpose: Safari answers any gesture-free `readText()` with its own native paste-confirmation prompt (so an auto-read would pop a system dialog on page load), and Firefox exposes no page-script read at all — the permission name is unknown there and the query throws, which is caught and treated as "no silent read". So on iOS the Paste button *is* the whole feature and the suggestion never appears; that's correct behaviour, not a bug. Telegram's `readTextFromClipboard` is tried first inside `pasteLink()` but is documented (Bot API 6.4) as returning real text only for Mini Apps launched from the attachment menu — every other launch fires the callback with an empty string, hence the fall-through to the browser API and the 1s timeout guard for clients where the callback never fires at all. **A bot cannot read a clipboard**: there is no Bot API surface for it, so "paste and send" in the bot chat is not implementable — the bot already parses any URL that's sent to it, which is the closest equivalent.
...
**OIDC and Mini App sign-ins are two different providers.** Web login is `telegram-oidc`, the Mini App is `telegram`. They only resolve to one user because `mapOIDCProfileToUser` stamps `user.telegramId` and the Mini App path looks users up by it. **Stamp the id_token's `id` claim, not `sub`** — Telegram's `sub` is an opaque pairwise identifier (a 20-digit value unrelated to the real id); only `id` matches the Mini App `initData` user id (and the bot's `message.from.id`). A user who signed in via OIDC **before** that mapping existed has `telegram_id = null` and will be duplicated on first Mini App launch — backfill from the id_token's `id` claim (gitignored `scripts/local/` script). Don't remove the `mapOIDCProfileToUser` mapping.
...
**Don't gate sign-in on the SDK object resolving — read `initData` from the launch hash instead.** Confirmed via PostHog: `telegram_mini_app_launched` (gated on `getTelegramWebApp()` returning non-empty) consistently never fired for deep-link launches (a `recipe_<id>` startapp param, `chat_type=private`) while firing reliably for plain reopens (`chat_type=sender`) — on iOS, in the traces seen. `TelegramProvider` first tried polling `getTelegramWebApp()` for ~5s to cover a suspected late-populating `initData`, but account creation still failed on deep links: the SDK object's `initData` apparently never resolves at all on that launch path, not just late. Silent throughout — no Sentry/Axiom signal, since nothing throws; `useTelegramAutoSignIn` just never attempted sign-in with an `undefined` webApp, and `ProfileAuth`'s `awaitingTelegramAutoSignIn` spun forever since `authStatus` never left `"idle"`. The actual fix: the `tgWebAppData` launch hash carries the *identical signed initData payload* `webApp.initData` would contain — `getLaunchInitData()` (`lib/telegram/webapp.ts`) reads it directly, available from the very first paint, so `useTelegramAutoSignIn` no longer needs the SDK object at all to attempt sign-in.

**…but the launch hash is gone by the time sign-in reads it on a deep link — remember it.** The hash fix above still left new users failing on the *shared-recipe* path specifically: `TelegramDeepLink` reads the `start_param` and immediately `navigate.push`es the recipe view, and that push (`history.pushState(null, "", href)` in `navigation-stack.tsx`) rewrites the URL and **drops `#tgWebAppData` within the same page load** — well before `useTelegramAutoSignIn`'s effect (which first waits on the async session check) gets to read it. On a `chat_type=private` launch the SDK webApp never resolves either (see above), so there's no `webApp.initData` fallback and `getLaunchInitData()` came back `undefined` → `authStatus` stuck `"idle"` → Profile skeleton forever, no account. Plain launches were unaffected: no recipe push wipes the hash and the webApp resolves anyway. Fix: `getLaunchDataFromHash()` now **remembers the decoded payload in `sessionStorage`** (`recipai:tg-launch-data`, mirroring the `recipai:tg-launched` flag beside it) the first time it's seen — `TelegramDeepLink`'s `getLaunchStartParam()` reads it while the hash is still intact, before its own push — and returns the remembered copy once the live hash is gone. Live hash always wins when present (and refreshes the memory). Diagnostic heuristic: sign-in works on a plain Mini App open but a **shared-recipe deep link** leaves Profile spinning = something consumed and discarded the launch hash before the sign-in read it.
...
**A brand-new Mini App user needs a placeholder email.** `user.email` is `NOT NULL UNIQUE`, but `better-auth-telegram`'s default Mini App mapper creates the user with `email: undefined` (Telegram provides no email). So a user whose **first** touch is the Mini App (never signed in via web/OIDC) fails the `user` INSERT and auto sign-in never completes — while existing users work, because the signin handler finds them by `telegramId` and skips the insert. Fix: pass `miniApp.mapMiniAppDataToUser` (`lib/auth/telegram-user.ts` → `miniAppDataToUser`) that synthesizes `<telegramId>@telegram.miniapp`, mirroring the OIDC path's `<sub>@telegram.oidc`. Same failure family as the `account`-columns gotcha above (validate-OK, signin-500 on an insert), but on the `user` table via a null column instead of a missing one. Diagnostic heuristic: **existing** Telegram users sign in fine but **new** ones can't = the new-user INSERT, i.e. the email placeholder.

**`signInWithMiniApp` doesn't refresh the reactive `useSession` store.** better-auth only auto-refreshes `useSession` for a fixed allow-list of paths (`/sign-in/email`, `/sign-out`, `/update-user`, …) via its `atomListeners`. `signInWithMiniApp` is a raw plugin endpoint (`/telegram/miniapp/signin`), so a successful sign-in sets the session cookie but **`useSession` stays `null` until the next full page load** — the symptom is "signed in only after I reopen the app," with no error (nothing failed). `authClient.getSession()` does **not** fix it: it fetches `/get-session` directly without toggling `$sessionSignal`, so it never updates the atom `useSession` reads. The fix is `authClient.$store.notify("$sessionSignal")` after a successful `signInWithMiniApp` (`components/telegram-provider/use-auto-sign-in.ts`) — it toggles the signal, `useAuthQuery` refetches, and (because the current data is `null`) that refetch reports `isPending`, which also covers any consumer gating on it. Consumers that show a signed-out prompt (e.g. `ProfileAuth`) must additionally treat Telegram `authStatus === "idle" | "pending"` as loading, or a new user — whose account creation makes the window longer — sees the sign-in prompt flash before it resolves.
